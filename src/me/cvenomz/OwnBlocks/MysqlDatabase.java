...
public class MysqlDatabase {
    ...
    public int addBlock(MysqlBlock mb) throws Exception
    {
        Statement s = conn.createStatement();
        //int ret = s.executeUpdate("INSERT INTO OwnBlocks (x, y, z, owner, allowed, tag) VALUES ("+x+","+y+","+z+",'"+owner+"','"+allowed+"','"+tag+"')");
        String value = mb.toSQLString();
        System.out.println("INSERT INTO "+tableName+" (world, x, y, z, owner, allowed, tag) VALUES " + value);
        int ret = s.executeUpdate("INSERT INTO "+tableName+" (world, x, y, z, owner, allowed, tag) VALUES " + value);
        return ret;
    }
    
    public MysqlBlock getBlock(MysqlBlock mb)throws Exception
    {
        Statement s = conn.createStatement();
    ...
    public int deleteBlock(MysqlBlock mb)throws Exception
    {
        Statement s = conn.createStatement();
        int ret = s.executeUpdate("DELETE FROM "+tableName+" WHERE world="+mb.getWorld()+" AND x="+mb.getX()+" AND y="+mb.getY()+" AND z="+mb.getZ());
        return ret;
    }
